 ...
 * seeds for all 100 streams are given in the co
 *
...
double lcgrand(int stream) {
    unsigned long zi, lowprd, hi31;

    zi = zrng[stream];
    lowprd = (zi & 65535u) * MULT1;
    hi31 = (zi >> 16u) * MULT1 + (lowprd >> 16u);
    zi = ((lowprd & 65535u) - MODLUS) + ((hi31 & 32767u) << 16u) + (hi31 >> 15u);

    if (zi < 0) zi += MODLUS;

    lowprd = (zi & 65535u) * MULT2;
    hi31 = (zi >> 16u) * MULT2 + (lowprd >> 16u);
    zi = ((lowprd & 65535u) - MODLUS) + ((hi31 & 32767u) << 16u) + (hi31 >> 15u);

    if (zi < 0) zi += MODLUS;
    zrng[stream] = zi;

    return (zi >> 7u | 1u) / 16777216.1;
}
...
unsigned long lcgrandgt(int stream) {
    return zrng[stream];
}
